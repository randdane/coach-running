...
```bash
git clone https://github.com/your-username/coach-running.git
cd coach-running
...
5.  **Get your Initial Refresh Token** by doing a one-time OAuth exchange:
    ...
    ```
    b. Authorize the app. You'll be redirected to a `localhost` URL — copy the `code=` value from it.
...
1.  Install [Tailscale](https://tailscale.com/download) on your host.
2.  Enable **Tailscale Funnel** for port 8000:
    ```bash
    tailscale funnel 8000
    ```
3.  Your public URL will be `https://<your-node-name>.<your-tailnet-name>.ts.net`.
4.  Generate a random secret for your webhook path:
    ```bash
    openssl rand -hex 16
    ```
5.  Set `WEBHOOK_SECRET` in your `.env` to that value.
6.  Register the webhook subscription with Strava (do this after the coach is running in Step 5):
    ```bash
...
2.  In the app, add a new server pointing to your Tailscale IP or hostname: `http://<tailscale-ip>:8080`.
